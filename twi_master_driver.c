...
//USART_data_t USART_data;                                          //Uncomment when using UART


void TWI_MasterReadDATA(TWI_Master_t *twi)                    //by VAibhav- data sent by slave to the master
{
	...
	if (twi->bytesRead < TWIM_READ_BUFFER_SIZE) {
		...
		twi->bytesRead++;
//		UART_TXBuffer_PutByte(&USART_data, data);	                             // send data received by slave via UART     added by me
		_delay_ms(100);
	}
